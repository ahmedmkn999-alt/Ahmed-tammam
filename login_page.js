...
export default function LoginPage() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      // نتحقق من الـ hash أو plain text (للطلاب القدام قبل التحديث)
      const passwordMatch =
      ...
      // لو الـ password لسه plain text، نحدثه لـ hash تلقائياً
      if (userData.password === password && userData.password !== hashedPassword) {
        try {
          const { updateDoc, doc: firestoreDoc } = await import("firebase/firestore");
          const { db: firestoreDb } = await import("@/lib/firebase");
          await updateDoc(firestoreDoc(firestoreDb, "students", phone.trim()), {
            password: hashedPassword,
          });
        } catch {}
      }

      // ✅ لو نفس الجهاز، نستخدم نفس الـ sessionId — بس لو جهاز جديد نعمل واحد جديد
      const existingUser = (() => {
        try { return JSON.parse(localStorage.getItem("currentUser")); } catch { return null; }
      })();
      const isSameDevice =
        existingUser?.studentPhone === phone.trim() ||
        existingUser?.phone === phone.trim();
      const sessionId = isSameDevice && existingUser?.sessionId
      ...
      // ✅ تسجيل الجلسة على السيرفر — يبطل أي جلسة قديمة تلقائياً
      ...
      // ✅ إعلام أي tab تانية في نفس المتصفح إن الجلسة القديمة انتهت
      try {
  ...
  async function handleResetPassword(e) {
    ...
    try {
      const userRef = doc(db, "students", resetPhone.trim());
      // ✅ hash كلمة السر الجديدة قبل الحفظ
      const hashRes = await fetch("/api/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
