...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 700px) {
    ...
    .tabs-bar { padding: 0 0.5rem; overflow-x: auto; gap: 0; }
    .tab-btn { padding: 0.7rem 0.7rem; font-size: 0.8rem; white-space: nowrap; }
  ...
  @media (min-width: 1024px) {
    .admin-main { padding: 2rem 3rem; max-width: 1300px; margin: 0 auto; }
        ...
        <div className="modal-overlay" onClick={() => setSelected(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            {[
              ...
              // ✅ كلمة السر محذوفة من العرض — حساسية عالية
...
export default function AdminPage() {
  ...
  useEffect(() => {
    // ✅ بنتحقق من الـ admin_session cookie اللي بيجي من الـ server
    // مش sessionStorage بس، لأن sessionStorage ممكن يتلاعب فيه
    async function verifySession() {
      try {
        ...
        // لو فشل الـ fetch، نرجع للـ sessionStorage كـ fallback
  ...
  async function handleLogin() {
    setLoginError("");
    try {
      // ✅ بنبعت الكلمة للـ API Route على السيرفر — مش بنقارنها في المتصفح
      ...
      if (res.ok) {
        ...
        setLoginError("كتير أوي محاولات، استنى 15 دقيقة وحاول تاني");
      } else {
