// app/courses/page.js  (أو أي مسار تحطه فيه)
// نفس الصفحة الأصلية + حفظ رسائل الدعم في Firebase + ربط أكواد التفعيل بـ Firestore
"use client";
...
export default function CoursesPage() {
  ...
  // ✅ null مش localStorage مباشرة — بيمنع الـ flash
  ...
  // ✅ Auth check + single-session enforcement
  useEffect(() => {
    setMounted(true);
    ...
    try {
      ...
      // ✅ BroadcastChannel: لو نفس المتصفح فتح tab تاني
      let bc = null;
      try {
        bc = new BroadcastChannel("session_channel");
        // إعلان إننا فاتحين
        bc.postMessage({ type: "SESSION_ACTIVE", sessionId: parsed.sessionId });
        bc.onmessage = (e) => {
          if (
            e.data?.type === "SESSION_KICKED" &&
            e.data?.sessionId !== parsed.sessionId
          ) {
      ...
      // ✅ Firestore polling: فحص كل 30 ثانية لو جهاز تاني (موبايل مختلف) دخل
      const phone = parsed.studentPhone || parsed.phone;
      const sessionId = parsed.sessionId;
      let interval = null;

      if (phone && sessionId) {
        let invalidCount = 0; // ✅ عداد لمنع الطرد الغلط بسبب network مؤقت
        ...
        async function checkSession() {
          try {
            ...
            if (!res.ok) { invalidCount = 0; return; } // لو الـ API فشل، منطردش — نستنى المرة الجاية
            const data = await res.json();
            if (data.valid === false) {
              invalidCount++;
              // ✅ لازم يجيب valid=false مرتين متتاليتين عشان نطرد — يمنع false positive
              if (invalidCount >= 2) {
                localStorage.removeItem("currentUser");
                document.cookie = "currentUser=; path=/; max-age=0";
                if (bc) bc.close();
              ...
              invalidCount = 0; // ✅ لو رجع valid، نصفر العداد
            }
          } catch {
            invalidCount = 0; // مشكلة في الـ network — منطردش، نكمل
          }
        }
        // أول فحص بعد 10 ثواني فقط (مش فوري)
  ...
  // التحقق من كود التفعيل عبر Firestore
  const verifyActivationCode = async () => {
    ...
    try {
      ...
      if (!codeSnap.exists()) {
        triggerToast("الكود غير موجود، تأكد منه!", "❌");
        return;
      }
      ...
      if (codeData.used) {
        triggerToast("الكود ده اتستخدم قبل كدا!", "❌");
        return;
      }

      if (codeData.courseId !== modalCourseId) {
        triggerToast("الكود ده مش لهذا الكورس!", "⚠️");
        return;
      }
  ...
  const enterCourseMock = () => triggerToast("مرحباً بك داخل الكورس! جاري تحميل المحاضرات...", "📚");

  // حفظ رسالة الدعم في Firebase
    ...
    <div className="app-wrapper" data-theme={theme} dir="rtl">
      <style jsx global>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .custom-toast { position: fixed; top: 20px; left: 50%; transform: translateX(-50%) translateY(-50px); background: var(--card-bg); border: 1px solid var(--neon-teal); box-shadow: 0 0 25px var(--neon-glow); padding: 1rem 2.5rem; border-radius: 14px; color: var(--text-white); font-weight: 700; z-index: 3000; opacity: 0; pointer-events: none; transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); display: flex; align-items: center; gap: 12px; }
        .custom-toast.show { opacity: 1; transform: translateX(-50%) translateY(0); }
        .courses-header { width: 100%; max-width: 480px; display: flex; justify-content: space-between; align-items: center; padding: 1.2rem 1rem; background: var(--bg-main); border-bottom: 1px solid rgba(255, 255, 255, 0.03); position: sticky; top: 0; z-index: 1000; }
        ...
        .sidebar-welcome .student-name { font-size: 1.3rem; font-weight: 800; color: var(--text-white); margin-top: 4px; }
        ...
        .main-container { width: 100%; max-width: 450px; padding: 1.5rem 1rem; flex: 1; }
        ...
        .courses-grid { display: flex; flex-direction: column; gap: 2rem; }
        ...
        .course-price { background: rgba(0, 255, 208, 0.1); border: 1px solid var(--neon-teal); color: var(--neon-teal); padding: 0.3rem 1rem; border-radius: 10px; font-weight: 800; font-size: 1rem; }
        ...
        .modal-content { background: var(--card-bg); border: 1px solid var(--input-border); border-radius: 24px; width: 100%; max-width: 400px; padding: 2rem 1.5rem; box-shadow: var(--card-shadow); text-align: center; }
        ...
        .stats-badge { background: rgba(0, 179, 255, 0.1); border: 1px solid var(--neon-blue); color: var(--neon-blue); padding: 0.3rem 0.8rem; border-radius: 8px; font-size: 0.85rem; font-weight: 700; }
        .footer-main { width: 100%; max-width: 450px; text-align: center; margin-top: 4rem; padding: 1rem; display: flex; flex-direction: column; align-items: center; gap: 2rem; }
          ...
          <div style={{ background: "var(--card-bg)", padding: "1.5rem", borderRadius: "20px", border: "1px solid var(--input-border)" }}>
            <form onSubmit={handleSettingsSubmit}>
              ...
              <div className="form-group">
                ...
                <input type="tel" value={currentUser?.studentPhone || "—"} readOnly style={{ opacity: 0.7 }} />
              </div>
              <div className="form-group">
                <label>رقم ولي الأمر (الأب)</label>
                <input type="tel" defaultValue={currentUser?.fatherPhone || ""} />
              </div>
              <div className="form-group">
                <label>رقم ولي الأمر (الأم)</label>
                <input type="tel" defaultValue={currentUser?.motherPhone || ""} />
              </div>
              <div className="form-group">
                <label>الرقم القومي</label>
                <input type="text" value={currentUser?.nationalID || "—"} readOnly style={{ opacity: 0.7 }} />
              </div>
      ...
      <div className={`payment-modal ${isModalOpen ? "active" : ""}`}>
        <div className="modal-content">
          ...
          <a href="https://wa.me/201128381838" target="_blank" rel="noopener noreferrer" className="whatsapp-btn-link">
            تواصل عبر الواتساب 💬
          </a>
      ...
      {sessionKicked && (
        <div style={{
          ...
          <div style={{
            ...
            textAlign: "center", border: "1px solid rgba(0,255,208,0.2)",
            boxShadow: "0 20px 60px rgba(0,0,0,0.8)"
          }}>
            <div style={{ fontSize: "48px", marginBottom: "16px" }}>⚠️</div>
            <h3 style={{
              color: "var(--text-white)", fontSize: "18px",
              fontWeight: "700", marginBottom: "12px", lineHeight: 1.5
            }}>
            ...
            <p style={{
              ...
              سيتم تسجيل خروجك الآن لأن حسابك فُتح على جهاز مختلف.
            </p>
            <button
