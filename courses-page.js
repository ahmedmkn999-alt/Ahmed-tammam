...
export default function CoursesPage() {
  ...
  // ✅ إصلاح الـ flash: نقرأ localStorage بس بعد الـ mount
  useEffect(() => {
    setMounted(true);
    ...
    try {
      ...
      // ✅ فحص الجلسة كل 30 ثانية — لو جهاز تاني دخل، نطرد الجهاز ده
      ...
      // ✅ استنى 5 ثواني قبل أول فحص — عشان الـ session تتسجل في Firestore الأول
      setTimeout(() => {
        checkSession();
    ...
    return () => { if (sessionCheckInterval) clearInterval(sessionCheckInterval); };
  }, []);
